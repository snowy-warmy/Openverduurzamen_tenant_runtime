...
// Optioneel: to, subject, heading en intro overschrijven de standaardwaarden.
// De adviesaanvraag van De Energie Raadgever gebruikt dat om de aanvraag met
// het snelle rapport naar een partner te sturen in plaats van naar LEAD_EMAIL.
export async function sendLeadEmail({ firstName, lastName, email, phone, message, sourcePage, config, meta, reportContext, attachments, to: toOverride, subject: subjectOverride, heading, intro }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) throw new Error("RESEND_API_KEY ontbreekt.");
  ...
  const rawTo = toOverride || process.env.LEAD_EMAIL || config?.contact?.email || "";
  ...
  const subject = subjectOverride || `Nieuwe contactaanvraag — ${fullName}`;
  ...
  <h2 style="margin:0 0 8px;font-size:18px;">${escapeHtml(heading || "Nieuwe contactaanvraag")}</h2>
  <p style="margin:0 0 14px;color:#475569;">${escapeHtml(intro || "Iemand heeft via de website om contact gevraagd.")}</p>
