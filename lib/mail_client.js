...
export async function sendLeadEmail({ firstName, lastName, email, phone, message, sourcePage, config, meta, reportContext, attachments }) {
  ...
  // BCC via MAIL_BCC_ADDRESSES (komma-gescheiden) — zelfde variabele als
  // de rapport-mail, zodat de eigenaar meeleest zonder in het To:-veld
  // zichtbaar te zijn voor het team.
  const sendArgs = {
    from,
    to,
    subject,
  ...
  const bcc = parseBccAddresses();
  if (bcc.length) sendArgs.bcc = bcc;
