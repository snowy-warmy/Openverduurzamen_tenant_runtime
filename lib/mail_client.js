...
export async function sendLeadEmail({ firstName, lastName, email, phone, message, sourcePage, config, meta, reportContext, attachments }) {
  ...
  // LEAD_EMAIL accepts a single address or a comma-separated list, e.g.
  //   LEAD_EMAIL=info@woonwijzerwinkel.nl,sales@woonwijzerwinkel.nl
  // All recipients receive the same email (visible to each other on the
  // To: line). If you'd prefer one of them to receive it privately, put
  // them in MAIL_BCC_ADDRESSES instead.
  const rawTo = process.env.LEAD_EMAIL || config?.contact?.email || "";
  const to = String(rawTo)
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  if (!to.length) throw new Error("LEAD_EMAIL ontbreekt en geen contact e-mail in config.");
  if (!process.env.LEAD_EMAIL) {
    // eslint-disable-next-line no-console
    console.warn("[lead] LEAD_EMAIL is niet gezet — val terug op config.contact.email:", to.join(", "));
  }
...
// Operationele alerts
//
// Opt-in via ORDER_ALERT_ADDRESSES (komma-gescheiden). Staat die env-var niet,
// dan gebeurt er niets — tenants die hem niet zetten merken van deze code
// helemaal niets.
//
// Waarom dit bestaat: bij een tenant die achteraf factureert is er geen
// betaling meer die bevestigt dat er een aanvraag liep. Mislukt het renderen
// of mailen, dan bleef dat hangen als orderstatus en zag niemand het. De BCC
// op de rapportmail dekt alleen het geval dat het wél goed ging.
// ---------------------------------------------------------------------------

function parseAlertAddresses() {
  return String(process.env.ORDER_ALERT_ADDRESSES || "")
    .split(",")
    .map((x) => x.trim())
    .filter(Boolean);
}

export function orderAlertsEnabled() {
  return parseAlertAddresses().length > 0;
}

const ALERT_KINDS = {
  requested: { label: "Aanvraag ontvangen", tone: "#0f766e" },
  failed: { label: "Aanvraag MISLUKT", tone: "#b91c1c" },
};

/**
 * Verstuur een korte operationele mail over één order. Gooit nooit: een
 * mislukte alert mag de orderverwerking niet omgooien.
 *
 * kind: "requested" | "failed"
 */
export async function sendOrderAlertEmail({ kind, order, config, extra = {} }) {
  const to = parseAlertAddresses();
  if (!to.length) return { sent: false, reason: "no_recipients" };

  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return { sent: false, reason: "no_api_key" };

  const meta = ALERT_KINDS[kind] || ALERT_KINDS.requested;
  const brand = config?.brand?.name || config?.id || "OpenVerduurzamen";
  const addr = order?.address || {};
  const addressLine = [addr.postalcode, addr.housenumber, addr.houseletter, addr.houseaddition]
    .filter(Boolean)
    .join(" ");
  ...
  const rows = [
    ...
    ["Order", order?.id || "—"],
    ["Adres", addressLine || "—"],
    ["E-mail klant", order?.email || "—"],
    ["Aangevraagd door", order?.source_meta?.internal_user || "—"],
    ["Bron", order?.source_meta?.source || "—"],
    ["Tijd", new Date().toISOString()],
  ];
  if (extra.error) rows.push(["Foutmelding", String(extra.error).slice(0, 500)]);
  if (extra.step) rows.push(["Laatste stap", String(extra.step)]);

  const html = `<div style="font-family:Arial,Helvetica,sans-serif;color:#0f172a;line-height:1.6;max-width:640px;">
  <p style="margin:0 0 4px;font-weight:700;color:${meta.tone};">${meta.label}</p>
  <p style="margin:0 0 14px;color:#475569;font-size:13px;">${brand} — automatisch bericht, niet beantwoorden.</p>
  <table style="border-collapse:collapse;font-size:13px;">
    ${rows
      .map(
        ([k, v]) =>
          `<tr><td style="padding:3px 14px 3px 0;color:#64748b;vertical-align:top;">${k}</td><td style="padding:3px 0;"><b>${String(v)
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")}</b></td></tr>`
      )
      .join("")}
  ...
  try {
    const resend = new Resend(apiKey);
    const { error } = await resend.emails.send({
      from: config?.mail?.fromAddress || `${brand} <no-reply@openverduurzamen.nl>`,
      to,
      subject: `[${brand}] ${meta.label} — ${addressLine || order?.id || ""}`.trim(),
      html,
    });
